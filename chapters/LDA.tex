...
\footnotesize
\textbf{1. Đầu vào:} X có kích thước $N \times D$, D rất lớn. $t_k$ là nhãn lớp, có C lớp khác nhau.

\textbf{2. Mục tiêu:} Giảm số chiều từ D xuống M $(M \leq C - 1)$ sao cho dữ liệu sau khi giảm có tính phân tách cao nhất.
}

\cheatbox{2. Bài toán tối ưu}{\textStrech}{
\footnotesize
...
\\ Tìm hướng $w^* = \arg\max_w J(w) \boxed{J(w) = \frac{w^T S_B w}{w^T S_W w}}$
}
...
\cheatbox{3. Trường hợp có C lớp}{\textStrech}{
\footnotesize
\textbf{1. Các thành phần:}
\begin{itemize}[leftmargin=*, nosep]
...
\begin{itemize}[leftmargin=*, nosep]
    ...
    \item \textbf{Vì:} Ma trận $S_B$ là tổng có trọng số của C ma trận có hạng là 1. Bất kỳ $m_k$ nào cũng có thể tính qua C-1 vector $m_k$ còn lại, và $N_k = N - \sum_{i = 1}^{C|i\neq k} N_i$.
\end{itemize}
