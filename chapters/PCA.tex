\cheatbox{1. Kiến thức Toán nền tảng}{\textStrech}{
\footnotesize
\textbf{1. Hình chiếu:}
...
\cheatbox{2. Giới thiệu \& Mục tiêu PCA}{\textStrech}{
\footnotesize
\textbf{1. Đầu vào:}
\begin{itemize}[leftmargin=*, nosep]
    \item Ma trận dữ liệu $X (N \times D)$ ($N$ mẫu, $D$ chiều). Thường $D$ rất lớn.
...
\begin{itemize}[leftmargin=*, nosep]
    \item Khi chiếu dữ liệu lên không gian con, phương sai của dữ liệu là lớn nhất: $argmax_u \frac{1}{N} \sum_{k=1}^N x^2_{u,k}$
    \item Hoặc khi chiếu dữ liệu lên không gian con, khoảng cách từ điểm dữ liệu đến không gian con là nhỏ nhất: $argmin_u \sum_{k=1}^N d^2_{u,k}$
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item Covariance matrix (ma trận hiệp phương sai): $S = \frac{1}{N} \sum_{n=1}^N (x_n - \mu)(x_n - \mu)^T =\frac{1}{N} Z^T Z =\frac{1}{N} (X - \mu)^T (X - \mu)$. S là ma trận vuông, đối xứng $D \times D$, bán định dương.
...
\begin{itemize}[leftmargin=*]
    ...
    \item PCA: Tính ma trận hiệp phương sai $S$ rồi phân rã eigen trên $S$. SVD: Phân rã SVD trên chính ma trận gốc (X), quá trình phân rã ổn định hơn.
    \item Ma trận X được SVD phân rã thành $X = U S V^T$. Với $U(N \times N)$: có cột là eigenvectors của $XX^T$. $V(D \times D)$: có cột là eigenvectors của $X^TX$. $S(N \times D)$: ma trận có đường chéo chính là singular values xếp từ lớn đến nhỏ. (\text{Singular Value là căn bậc 2 của eigenvalue})
\end{itemize}
...
\begin{itemize}[leftmargin=*]
    ...
    \item Chọn M vectors đầu tiên của V tương ứng M singular values lớn nhất tạo $\hat{V}$. 
...
\begin{itemize}[leftmargin=*, nosep]
    ...
    \item \textbf{Với:} A: ma trận hiệp phương sai có hạng là r. Thu giảm chiều: Chỉ giữ lại M eigenvector và chiếu X lên các vector này.
\end{itemize}

\textbf{SVD:}
\begin{itemize}[leftmargin=*, nosep]
    \item $A = U S V^T = \sum_{i=1}^{r} s_i u_i v_i^T$.
    \item A: ma trận bất kỳ có hạng là r. Thu giảm chiều: Chỉ giữ lại M eigenvector và xấp xỉ X lên các vector này.
\end{itemize}
}
